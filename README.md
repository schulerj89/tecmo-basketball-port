...
The `intro-arena-transition` and `intro-arena-frameN` render modes consume the ignored local Lua watcher output when available. The current native arena slice uses screen id `$18` nametable writes from frames 428-430, palette fade writes from frames 425-476, live scoreboard OAM from frame 461, `$2005` scroll-pair evidence for the vertical arena transition, MMC3-style 1KB R0/R1 background CHR slot resolution, and a composite row stream that places the small pixel crowd/stairs above the larger foreground crowd/court band; no ROM, ASM, or extracted graphics are committed.
