...
Generated from committed JSON at `2026-08-03T22:34:04Z`. This dashboard reports coordination state only; it is not product QA.
...
| LANE-R2-SHOTS-OUTCOMES | gameplay_behavior | active | ready | R2-SHOTS-OUTCOMES | S-SOL-R2-SHOTS-001 | codex/r2-shots-outcomes-sol | First commit remains held. Same Luna must close exact selector 0/3/2/1 miss schedules, ordinary approximate make, numeric0/1/2 close make+miss settlement, route-bound active-tail duration/JUMP base-frame invariants, and coordinated corruption probes, then pass Sol warning-clean, focused, full-scene, and native-proof gates. source_map.c and scene.c remain zero-diff exclusions. |
