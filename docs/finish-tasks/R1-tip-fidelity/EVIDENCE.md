...
| Sample-derived human countdown | Bank04 `$8795-$87D0` transforms the captured `$8A` byte into `abs($F9-captured)`, capped at `$0B`; `$0C` is retained only for no sample. Native C performs the same wrapping-byte subtraction/absolute-value seam and routes the derived countdown with the sampled side. |
| Cinematic jumper continuity | Bank05 `$A274-$A2DE` changes slot-10/receiver/event state without clearing either jumper, while `$86BB/$86DD/$8732/$8745` own animation, airborne physics, landing, and recovery. Native C snapshots implicitly by retaining the full actor tuple, pauses jumper dispatch while the cutaway overlay is active, displays the unchanged tuple on court return, then resumes ballistic recovery (including the few remaining updates after LIVE handoff). The exact overlay scheduling is classified native-faithful from the user's ROM observation; the definite no-reset/normal-landing contract is source-exact. |
