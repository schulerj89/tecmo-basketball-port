...
| TGFT live-tick coupling | `native_approximation_with_justification` | The existing production scene already bridges fixed actor slots/active lists into the caller-owned kernel outside this candidate. | Keep the kernel policy-free and transactional; dynamic active-lineup/substitution ownership, replacement, and timing remain deferred. | Focused vectors prove the kernel contract; the fixed-slot bridge exists, while dynamic replacement behavior is not claimed. |
| TGFT parse/load | `native_faithful` | Native asset boundary and C alias safety. | Valid preloaded assets survive malformed/NULL replacement. Old-storage payload alias is staged safely; assets-object/storage overlaps reject before writes; bounded corrupt old storage is not freed during replacement. | Sentinel object/storage tests and two successful loads. |
...
| TGFL parse/load | `native_faithful` | Native asset replacement boundary. | Failed replacement preserves a valid old object; struct aliases reject before writes; bounded corrupt old storage is not freed during replacement. | Preloaded old-storage malformed replacement, NULL reload, assets/payload/core aliases. |
...
| Production stable slots/native selection | `native_approximation_with_justification` | Current scene supplies fixed actors `[0..4]/[5..9]`, roster indexes, active flags, launch starter arrays, validation, and the existing fixed-slot bridge. | This worker does not reinterpret those stable slots or select dynamic replacements; preserving that current boundary avoids inventing ownership. | Only dynamic active-lineup/substitution ownership requires the exact scene/game-flow rescope in `APPROXIMATIONS.md`. |
...
| Fixed-scene active lists | `incomplete` | Production scene’s current fixed active lists/actor bindings are already bridged at the scene boundary. | TGFT accepts caller-provided unique 2x5 lists but does not derive dynamic replacements or substitutions. | Dynamic active-list replacement/ownership is deferred. |
