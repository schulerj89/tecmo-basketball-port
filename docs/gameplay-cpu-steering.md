...
| `$8F2D-$8F71` opcode 6 | Selected-primary automatic prepass only. Exact record offset 400 / `$A0BE` (`06 00 00 00 00`); no controller owns the possessing team. | Automatic `$8F2D-$8F4C` retains `$0190`/state 4, writes typed object-10 state `$13` and actor action `$10`, and toggles the persistent typed `$0588` bit-0 latch. The next AI update consumes action `$10` through fixed `$C711` selector 1 into the existing `$89DB/$89D7` pass gather. Controlled `$8F4D-$8F71` remains deferred; `$0743=0` remains unclaimed. |
| `$8ED7-$8F11` opcode 8 | Immutable held-ball X and TGOR orientation. The eight exact records are `$0B68/$0BA4->$025D`, `$0B77/$0BB3->$029E`, `$0B86/$0BC2->$02DA`, and `$0B95/$0BD1->$030C`. Each redirect destination begins with opcode **decimal 17**, not opcode 11. | Orientation 0 redirects below `$0140`; orientation 1 redirects at or above `$01C0`. Redirect stores C8:C9 directly with state 4 and no +5 and preserves `$0588`; the complement writes state 4, clears the persistent typed `$0588` bit-0 latch via AND `$F8`, and advances +5. |
...
| `$842E-$8471` autonomous shot predicate | The fixed-frame context owns `$0798/$075F/$0760/$006A`; TTDT profile byte 4 owns `$0533`; admitted `$9DF6` producers retain the exact `$8545` max-plus-half-min metric. The ordinary-LIVE lifecycle owns `$BA=0` and `$0478=0`, while a valid same-frame `$B721/$B783->$A023` context rejects the zero-state admission after pass/contact settlement. `$0588` bit 0 is sampled immutably before traversal from the persistent latch seeded by `$85F1` and updated by admitted `$8F37/$8EFB/$9029` paths. | Exact for those inputs and tested at the `$8545<$0010` boundary, set-bit rejection, nonzero same-frame `$0478` rejection, pre-traversal opcode-6 ordering, and made-score restart set. The two clearing writer families (`$B235/$BAB3`) remain fail-closed until their caller lifetimes are bound; unsupported shot playback remains deferred/non-launch. |
