...
| `$9032-$9052` opcode 20, raw `$038D:$038E/$038F:$0390` latch words | Typed harness/test input only, shared with opcode 13. Exact records are `$000F` / CPU `$9F3D` and `$0019` / `$9F47`, both `14 00 00 00 00`; actor-command assignment naturally supplies the preceding `$000A` or `$0019` stream, but not the latch. | `missing-global-target` in production. With a typed test latch it computes wrapping raw deltas, preserves target-plane bits under explicitly inactive storage provenance, clears their former semantic/raw meaning, applies the exact zero-vector/state-4 or nonzero-direction behavior, and unconditionally advances +5 without reading `$BA`. Inactive storage is never published as a movement target and is replaced by the next real target write. |
