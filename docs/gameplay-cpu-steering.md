...
| `$9125` opcode 13, raw `$038D:$038E/$038F:$0390` latch words | TGGL-1 types all five Bank05 writer families, atomic last-writer-wins serial/provenance, fixed full-reset clear, and period/possession retention. Both high bytes are live. The package is evidence-only: object/event scheduling, `$A214`, opcode 15, and `$0041` latest-writer timing are not bound, and the scene supplies no ball/current-position/TGCA substitute. | `missing-global-target`. An intentional test snapshot still separately requires the ordinary-LIVE `$BA&3==0` seam. |
