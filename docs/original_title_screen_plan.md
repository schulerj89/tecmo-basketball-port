...
- The native launcher now exposes an Intro Lab mode that renders the selected local-only CHR sheet and assembles `TECMO PRESENTS` through the Bank 06 character map against the selected CHR bank/table for placement QA. Bank 31 table 0 is only the default starting point, not an intro-source assertion.
