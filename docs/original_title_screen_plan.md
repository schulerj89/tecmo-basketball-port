...
- The native runtime now boots into the current CHR-backed title diagnostic; Enter opens the launcher and X quits.
...
- The native launcher now exposes a CHR Playground mode that renders the selected local-only CHR bank/table, tile IDs `$080-$0AF` or `$180-$1AF`, the assembled 2x2 title glyphs, and sample glyph tile mappings for visual QA. It defaults to Bank 31 table 0 because that is the verified title glyph path, but it can switch banks and pattern-table halves at runtime.
- The native launcher now exposes an Intro Lab mode that renders the selected local-only CHR sheet, lets the user place selected source tiles onto the target canvas, shows local placement records, and can save ignored placement JSON. Bank 31 table 0 is only the default starting point, not an intro-source assertion.
