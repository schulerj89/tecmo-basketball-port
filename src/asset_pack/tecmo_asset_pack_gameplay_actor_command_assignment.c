...
static const uint8_t a023_rev1_sha256[32] = {
    ...
    const uint8_t *bytes)
{
    ...
    case TECMO_GAMEPLAY_ACTOR_COMMAND_ASSIGNMENT_SOURCE_OBJECT_STATE17:
    ...
    size_t message_size)
{
    ...
    if (rom == NULL || payload == NULL || provenance == NULL ||
        ...
        memcmp(input_sha256, a023_rev1_sha256, sizeof(input_sha256)) != 0 ||
    ...
    memcpy(payload + 40U, a023_rev1_sha256, sizeof(a023_rev1_sha256));
    ...
    for (index = 0U;
        ...
        if (source->bank >= prg_banks ||
            ...
            tecmo_asset_pack_fnv1a32(
                ...
                source->fingerprint_fnv1a32 ||
            fnv1a64(rom + (size_t)offset, source->byte_count) !=
                source->fingerprint_fnv1a64 ||
            !source_semantics_valid(source, rom + (size_t)offset)) {
