                                             ...
                                             const TecmoTeamDataProvenance *p)
{
    ...
    *first = 0;
    return tecmo_asset_pack_append_text(
        ...
        "{\"role\":\"full-chr\",\"source_entry\":\"chr/all\",\"source_offset\":%llu,\"size\":262144,\"fingerprint_fnv1a32\":\"F6F6E854\",\"fingerprint_fnv1a64\":\"96A64F53B240ABB4\"}],"
        "\"native_contract\":{\"payload_size\":%u,\"payload_fingerprint_fnv1a32\":\"812628F0\",\"screens\":3,\"selector_entries\":29,\"teams\":29,\"real_team_logos\":27,\"logo_cell_limit\":60,\"profile_palette_groups\":4,\"players_per_team\":12,\"player_stride\":184,\"portrait_cells\":24,\"static_shooting\":{\"attribute_indexes\":[4,5,6],\"multiplier\":4,\"visible_decimal_digits\":3,\"ownership\":\"selected-ttdt-player\",\"digit_ppu_cells\":[\"21E1\",\"21E5\",\"21E9\"],\"digit_logical_origins\":[[8,120],[40,120],[72,120]],\"authored_dot_ppu_cells\":[\"21E0\",\"21E4\",\"21E8\"],\"authored_dot_tile_origins\":[[0,120],[32,120],[64,120]]},\"detail_physical_layout\":{\"height_ppu_start\":\"20D6\",\"height_origin\":[176,48],\"weight_ppu_cells\":\"20F6-20F8\",\"weight_right_edge\":200,\"weight_y\":56,\"position_ppu_start\":\"2116\",\"position_origin\":[176,64]},\"value_palette\":{\"ownership\":\"bank00-877d-screen-attributes\",\"renderer\":\"screen-2-per-cell\",\"value_row\":15,\"value_palette_index\":3,\"label_row\":13,\"label_palette_index\":2,\"dynamic_matches_authored_dot\":true},\"mutable_stat_layout\":{\"value_y\":120,\"integer_ppu_spans\":[\"21EE-21F1\",\"21F2-21F5\",\"21F6-21F9\"],\"integer_right_edges\":[144,176,208],\"points_ppu_span\":\"21FA-21FE\",\"points_decimal_ppu_cell\":\"21FD\",\"points_decimal_tile\":\"81\",\"points_right_edge\":248},\"gameplay_uniform_colors\":{\"home_table_entries\":29,\"away_default\":48,\"away_lakers\":56,\"lakers_team_id\":12},\"roster_rows\":6,\"roster_pages\":2,\"slide_frames\":32,\"slide_pixels_per_frame\":8,\"cursor_coordinates\":{\"generic_delta\":[0,-4],\"profile\":{\"config\":12,\"emitter\":[135,84],\"resolved_oam\":[135,80],\"visible\":[135,81]},\"roster\":{\"config\":16,\"emitter\":[40,147],\"resolved_oam\":[40,143],\"visible\":[40,144]},\"row_stride\":8,\"visible_y_rule\":\"oam-y-plus-one\"},\"entry_transition\":{\"render_on\":4,\"first_visible\":7,\"palette_step\":4,\"stable\":20},\"selector_profile_transition\":{\"black\":8,\"render_off\":10,\"render_on\":16,\"first_visible\":19,\"palette_step\":4,\"stable\":32},\"roster_detail_transition\":{\"black\":8,\"render_off\":10,\"render_on\":15,\"first_visible\":18,\"palette_step\":4,\"stable\":31},\"profile_roster_transition\":\"oam-only-stable-next-frame\",\"all_star_name_mapping\":\"direct-player-pointer-to-real-roster-slot\",\"terminal\":\"player-detail-no-gameplay\"}}",
    ...
    const TecmoGameplayCameraProvenance *p)
{
    ...
    *first = 0;
    ...
    return tecmo_asset_pack_append_text(
        ...
        "\"update\":\"exactly one route-zero follow after all live object and ball mutations; focus is ball world X and orientation is TGOR attack_direction\","
    ...
    const TecmoGameplayMovementProvenance *provenance)
{
    ...
    *first = 0;
    ...
    return tecmo_asset_pack_append_text(
        ...
        "\"cpu_target_and_shot_policy\":\"exact $842E frame, $8545 metric, ordinary $BA zero, same-frame-aware $0478, and immutable pre-traversal $0588 bit0 from $85F1/$8F37/$8EFB/$9029; two Bank05 clearing families remain fail-closed; unsupported playback is deferred\"},"
    ...
    const TecmoGameplayCpuSteeringProvenance *provenance)
{
    ...
    *first = 0;
    ...
    return tecmo_asset_pack_append_text(
        ...
        "\"scope\":\"typed persistent provenance plus bounded A0F3/A790/A9DA shot LIVE chain; general global target remains unavailable\","
        ...
        "\"record_002d\":\"anchored $A9DA->$A993 assignment linkage; bound for the frame-89 ordinary shot same-update traversal after persistent A0F3/A790 writes\","
        "\"record_0041\":\"bounded unreachable: zero direct seeds across all 46 pinned formations and no over-approximating command-graph path after opcode15 exact no-advance returns\","
        ...
        "\"ordering\":\"A9DA state10/B3DD/latch writes precede BA/05A1 aborts; 0588 bit7 precedes only 05A1; successful first same-loop $002D and linked $005A stores project native last-step bookkeeping, not source writes\","
        ...
        "\"a9da_object_side_effects\":\"$0478=$10; fixed $B3DD writes $0067=$F0/$0068=$02 and X=$0A copies them to $04A4/$04AF; successful BA-low2 clear writes $0588|=$80\","
        "\"presentation_tail_boundary\":\"$4010/$4012/$4013/$4015 DMC register writes are audio/presentation, not CPU assignment state\","
        "\"production_boundary\":\"authoritative TGLS raw flight velocity feeds A0F3/A790/A9DA latch writers; frame 89 commits A9DA state10/B3DD/0588 side effects and opcode13 assignment, then the grounded next A214 dispatch reaches B6E5->B721->A023 with a same-update opcode20 latch\"},"
        ...
        "\"division\":\"signed numerator times 64 over unsigned duration, full raw16 quotient with wrapped sign restoration; divisor zero alone yields 0000/7FFF/8001; A0F3 wrapping doubles quotient\","
        ...
        "\"missing_live_owners\":[\"$A214 states other than bound state10 B721, state17 B783, and pass state18 B7B6\"]},"
        ...
        "\"supported_raw_branches\":[\"$9187-$91C1 primary replacement\",\"$91C8-$9216 selected defender\"],"
        "\"exact_no_advance_returns\":[\"$9185 D0 F2 -> $9179 RTS\",\"$91C6 D0 B1 -> $9179 RTS\"],"
        "\"classified_deferred\":[\"missing raw owner\",\"invalid $0463 direction\",\"LIVE producer unavailable\"],"
        "\"primary_lifecycle\":{\"handler\":\"$9187-$91C1\",\"handler_fnv1a32\":\"B33C7281\",\"old_primary_reset\":\"$88B0-$88D9\",\"invalidation\":\"$C060->$CBF7 sets $058B/$058C=$FF\",\"invalidation_fnv1a32\":[\"1CDA0ECD\",\"EA90E12C\"],\"formation\":[{\"address\":\"$938B-$9403\",\"fnv1a32\":\"4CB55522\"},{\"address\":\"$943B-$946E\",\"fnv1a32\":\"82C361B6\"}],\"intermediate_handoff\":\"$9208 state7/$059E/selector4\",\"final\":\"$046E=1B;$057C=0;$000E[$030A]=new\"},"
        "\"selection_059e_lifecycle\":{\"sole_writer\":\"$920D\",\"consumer\":\"$9248-$926F\",\"consumer_fnv1a32\":\"96DD94EF\",\"reset\":\"fixed $CC58 page clear\",\"persistence\":\"never consumer-cleared; stale until next writer; full reset only\",\"validity\":\"independent dispatch actor state $057C==7 before $9248 loads stored actor\",\"dispatch\":\"selector3 with side derived from stored actor slot\",\"retirement\":\"stored actor != $0308 and $046E==0 clears stored actor $057C\"},"
        ...
        "\"live_missing_raw_reason\":\"generic play-step callers without the shot scheduler defer; no shadow mirrors\","
        "\"natural_fceux_capture\":\"shot-scoped opcode15 and slot-10-height traces corroborate the bounded LIVE owner; deterministic tests force both replacement branches\"},"
        "\"native_contract\":{"
        ...
        "35809,36722]},"
        "\"opcode21_gate\":{"
        "\"handler\":\"Bank06 $8BF6-$8C17\","
        "\"bit1_writer\":\"fixed $F07E-$F0B9\","
        "\"bit1_writer_size\":60,\"bit1_writer_fnv1a32\":\"F92274FB\","
        "\"predicate\":\"clear each loop; slot10 $0478==0; primary depth $7B..$AE; orientation0 X<$00F8 or orientation1 X>=$0208 sets bit1\","
        "\"typed_owners\":[\"ordinary LIVE slot10 state 0\",\"primary raw position\",\"$035A orientation\",\"$058A shot clock\",\"$0357/$0358 game clock\"],"
        ...
        "\"x_delta\":\"$0073/$00E8 target minus selected actor with 16-bit borrow\","
        "\"depth_delta\":\"$00F3 target minus selected actor, 8-bit result sign-extended through $A7\","
        ...
        "\"production_adapter\":\"scene_cpu_build_play_input captures the current Q8 ball snapshot once, advances the command once, and launches LIVE state 5 without TGMO retargeting\"}},"
        "\"planar_route_kernel\":{"
        ...
        "\"production_bound\":true,"
        "\"production_profile\":\"exact $C045 actor/lineup row mapping and wrapping Bank02 $06E7 arithmetic with caller-supplied typed live condition; optional $A908 admission uses an explicit typed no-controller native approximation rather than claiming raw $030C/$030D parity\","
        "\"completion_owner\":\"scene clock_divider is the typed $0359 decrement/reload lifecycle and supplies bit 0 after the normal pre-AI state update; this typed order does not claim complete original cross-bank intra-frame parity\","
        "\"lifecycle\":\"active route requires state 5 plus a frozen valid source target; target-invalidating role/formation transitions cancel route state transactionally\","
        "\"remaining_boundaries\":[\"raw $030C/$030D extra-adjust admission parity\",\"pose/action presentation side effects\"]},"
        ...
        "\"live_wired\":true,\"transactional\":true},"
        "\"evidence_limits\":{"
        ...
        "\"next_integration\":\"convert omitted route pose/action side effects and replace the typed extra-adjust admission approximation when raw $030C/$030D lifecycle ownership is proven\"},"
        ...
        "\"scope\":\"bound production LIVE scene launch and normal game flow; ordinary compatibility targets still compose TGAI-to-TGMO, while canonical opcode 4 launches the exact TGAI-3 state-5 planar route and bypasses TGMO for active route ticks\","
        ...
        "\"step_budget\":1,\"actor_order\":[9,8,7,6,5,4,3,2,1,0],"
        "\"source_command_advance\":true,\"deferred_effects_explicit\":true},"
        "\"immutable_ten_actor_snapshot\":true,\"transactional\":true,"
        "\"source_target_policy\":\"valid source actor references require a valid recorded coordinate; current LIVE compatibility movement may re-evaluate ordinary source targets each post-human snapshot, but canonical opcode 4 captures once and the bound state-5 kernel never retargets\","
        "\"state5_route\":{\"live_wired\":true,\"selected_primary_first\":true,\"ordinary_order\":[9,8,7,6,5,4,3,2,1,0],\"selected_defender_excluded\":true,\"tgmo_bypassed\":true,\"target_frozen\":true,\"transactional_lifecycle_cancel\":true},"
        ...
        "\"shot_request_adapter\":\"exact $842E fixed-frame, persistent $8545 metric, ordinary $BA zero, same-frame-aware $0478, and immutable pre-traversal $0588 bit0 from $85F1/$8F37/$8EFB/$9029; $B235/$BAB3 remain fail-closed; unsupported playback deferred/non-launch\","
    ...
    const TecmoGameplayShotResolutionProvenance *p)
{
    ...
    *first = 0;
    ...
    return tecmo_asset_pack_append_text(
        ...
        "\"native_entrypoint\":\"terminal miss claimant plus exact ordinary defense $9FC3/$9FF1/$BA65 caller\","
        "\"typed_effects\":\"$0308/$0309 save/promote; conditional $030A/$030B swap; "
        "$9042 X=9..0 $04B0 bit-$10 XOR; conditional descending $06CB scan; "
        "$B98B candidate remap; automatic-offense $0547/$0551=$007D and $057C=$04; "
        "defense caller exact $0588 masks, $0743/$0587/$07DE clears, $07E2=$14, wrapping $0756, and raw C042 slot-6 request\","
    ...
    const TecmoGameplayPenaltyProvenance *p)
{
    ...
    *first = 0;
    ...
    return tecmo_asset_pack_append_text(
        ...
        "\"presentations\":{"
        "\"foul\":{\"banked_dispatcher_selector\":34,\"lead_in_frames\":4,\"maximum_wait_frames\":160,\"presentation_sfx_id\":6,\"presentation_sfx_delay_frames\":16,\"live_restart_sfx_id\":5,\"live_restart_music_id\":5,\"live_restart_requires_game_music\":true,\"bank02_overlay\":\"$B0F8-$B398 class/type/name/number/counters; bonus is a side mask, not a verified BONUS string\"},"
    ...
    const TecmoGameplayViolationRefereeProvenance *p)
{
    ...
    *first = 0;
    ...
    return tecmo_asset_pack_append_text(
        ...
        "\"foul_fixed_script\":\"fixed $E95E-$EA11 selector $2C then selector $22\","
        "\"foul_overlay\":\"Bank02 $B0F8-$B398\","
        "\"foul_overlay_presentation\":{"
        "\"scope\":\"ordinary retained defensive pushing identity only\","
        "\"writer\":\"Bank02 $B0F8-$B398\","
        "\"class\":{\"ppu\":\"$2283\",\"text\":\"DEFENSIVE FOUL \"},"
        "\"kind\":{\"ppu\":\"$2292\",\"text\":\"PUSHING\"},"
        "\"number\":{\"ppu\":\"$22A3\",\"source\":\"TTDT roster packed BCD via $AF7D\"},"
        "\"name\":{\"ppu\":\"$22A6\",\"source\":\"TTDT roster name via $AFDC/$AFF0\",\"separator\":\"one source PPU read leaves the name blank cell\"},"
        "\"personal\":{\"ppu\":\"$22E3\",\"ordinals\":[\"1ST\",\"2ND\",\"3RD\",\"4TH\",\"5TH\",\"6TH\"],\"suffix\":\" PERSONAL FOUL\"},"
        "\"team\":{\"ppu\":\"$2303\",\"suffix\":\" TEAM FOULS\"},"
        "\"fouled_out\":{\"ppu\":\"$234B\",\"text\":\"FOULED OUT\",\"condition\":\"individual after >= 6\"},"
        "\"bonus\":\"$B373-$B398 changes the side mask at regulation 5/overtime 4; no visible BONUS label is claimed\","
        "\"timing_limit\":\"Bank02 dynamic PPU completion frame is unavailable; native renderer reuses TGVR capture-bounded blackout/fade\"},"
        "\"foul_dispatcher_selector\":34,"
        "\"foul_controller_selector\":0,"
        "\"foul_sequence_id\":0,"
        "\"foul_groups\":[1,2,2,2],"
    ...
    const TecmoGameplayFreeThrowLineupProvenance *p)
{
    ...
    *first = 0;
    ...
    return tecmo_asset_pack_append_text(
        ...
        "\"orientation_source\":\"TGOR-1 attack_direction synchronized from free-throw scoring-team possession\","
    ...
    const TecmoGameplayPreTipProvenance *provenance)
{
    ...
    *first = 0;
    ...
    return tecmo_asset_pack_append_text(
        ...
        "\"tip_input\":\"Bank04 captures the first held NES B level routed by team with error 0..11 or no-sample 12; Bank05 consumes the latch only after live ball height/countdown gates, while automatic actors use only the strict evolving-height $839F predicate and no synthetic target frame\","
        "\"tpti_mechanics\":{\"version\":3,\"magic\":\"TPM2\",\"capture_error\":\"B04 $86E1-$8817 exact clocked capture/error, intentionally overlapping the stored closeup entry\",\"shared_dispatch\":\"B05 $8351-$839E actor-state/$839F and shared-state/$8642 paths; $8642 is a false friend, not slot-10 TIP logic\",\"automatic\":\"B05 $839F strict live ball-high > $3D+(($6A & 31)>>2), with zero-control human/countdown path\",\"opposing\":\"B05 $9824-$984F opposing dispatcher and $985E-$988E opposing selected-actor path; the exact $985E-$986A TIP_INPUT subspan remains a separate 13-byte overlap\",\"jump_commit\":\"B05 $9C79-$9CC8 actor commit at $9C7F, source-pinned $8D92 helper, actor state $0B\",\"slot10_claim\":\"B05 $A214-$A2DE equality deferral and ball-minus-jumper < $3A, commit $0478=$17 at $A2D2 with $A2D5 post-store seam\",\"trajectory\":\"exact $BDF7-$BEF6 table plus $B32C/$BCF4 signed Q10.6 solver, $AA84 arithmetic halves, dynamic $0513/$051E count, $B500/$BD6E planar integration, and $B678 gravity\",\"later_settlement\":\"B05 $98E1-$9A5F is later/general collision settlement, not TIP claim\",\"ordering\":\"fixed $E537-$E542 proves high-bit ordering into $0758; fixed $E56E is a one-byte executable hook anchor only, with recurring/handoff labels mapper-gated\",\"rng\":\"fixed $CD96-$CDAB exact $6A mix seam; jumper $8D92 inputs from $0529/$7C48 remain a bounded native-faithful boundary\"},"
        ...
        "\"tip_animation\":\"both TPTI-selected actors advance independent $0B/$0C ballistic state beneath the presentation; class-specific phases 2/3/4 and landing poses are orientation encoded, while anchors and live handoff remain transactional\","
