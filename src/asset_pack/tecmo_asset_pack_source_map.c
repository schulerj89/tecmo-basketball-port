    ...
    const TecmoGameplayCpuSteeringProvenance *provenance)
{
    ...
    *first = 0;
    ...
    return tecmo_asset_pack_append_text(
        ...
        "\"production_boundary\":\"authoritative TGLS raw flight velocity feeds a persistent A0F3 target write, A7A9 performs the A790 object-position overwrite with traced BA-low2 zero admission, and A8E9/A9DA performs the projected overwrite; ordinary shot off-ball opcode13 consumes the latest admitted value and the A9DA-chosen $002D actor consumes it in the same update; $0041 remains unbound\"},"
