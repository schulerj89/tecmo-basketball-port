...
#include "asset_pack/tecmo_asset_pack_team_management.h"
...
static int add_native_arena_intro_entries(TecmoAssetPackBuilder *builder,
                                          ...
                                          TecmoGameplayBackcourtProvenance *backcourt_provenance,
                                          ...
                                          size_t message_size)
{
    ...
    if (enforce_finale_revision_fingerprints != 0 &&
        tecmo_asset_pack_build_gameplay_dunk_cutaway(
    ...
    if (enforce_finale_revision_fingerprints != 0 &&
        tecmo_asset_pack_build_gameplay_shot_resolution(
            rom, rom_size, prg_offset, prg_banks,
            enforce_finale_revision_fingerprints,
    ...
    if (enforce_finale_revision_fingerprints != 0 &&
        tecmo_asset_pack_build_gameplay_backcourt(
            rom, rom_size, prg_offset, prg_banks,
            enforce_finale_revision_fingerprints,
            backcourt_payload, sizeof(backcourt_payload),
            backcourt_provenance, message, message_size) != 0) {
        return -1;
    }
    if (enforce_finale_revision_fingerprints != 0 &&
        tecmo_asset_pack_build_gameplay_pretip(
            rom, rom_size, prg_offset, prg_banks,
            enforce_finale_revision_fingerprints,
            pretip_payload, sizeof(pretip_payload), pretip_provenance,
            message, message_size) != 0) {
        return -1;
    }
    if (enforce_finale_revision_fingerprints != 0 &&
    ...
    if (enforce_finale_revision_fingerprints != 0 &&
        tecmo_asset_pack_build_gameplay_hud(
            rom, rom_size, prg_offset, prg_banks,
            enforce_finale_revision_fingerprints,
            gameplay_hud_payload, sizeof(gameplay_hud_payload),
            gameplay_hud_provenance, message, message_size) != 0) {
        return -1;
    }
    ...
    }
    {
        ...
        int add_result;
        if (tecmo_asset_pack_build_music(rom, rom_size, prg_offset, prg_banks,
                                         1, &music_payload,
                                         &music_payload_size,
                                         music_provenance,
                                         message, message_size) != 0)
            return -1;
        entry_info = tecmo_asset_pack_make_entry_info(
            TECMO_ASSET_PACK_MUSIC_ID, TECMO_ASSET_PACK_TYPE_DATA,
            TECMO_ASSET_PACK_MUSIC_BANK, TECMO_ASSET_PACK_MUSIC_DIRECTORY_CPU,
            music_provenance->directory_offset,
            TECMO_ASSET_PACK_FLAG_DERIVED | TECMO_ASSET_PACK_FLAG_LOCAL);
        add_result = tecmo_asset_pack_builder_add_memory(
            builder, &entry_info, music_payload, music_payload_size,
            message, message_size);
        free(music_payload);
        if (add_result != 0) {
            tecmo_asset_pack_set_message(message, message_size,
                                         "Could not write native music entry.");
            return -1;
        }
    }
    {
    ...
    if (tecmo_asset_pack_build_team_management(
            rom, rom_size, prg_offset, prg_banks, chr_size,
            enforce_finale_revision_fingerprints,
            team_management_payload, sizeof(team_management_payload),
            team_management_provenance, message, message_size) != 0)
        return -1;
    entry_info = tecmo_asset_pack_make_entry_info(
        TECMO_ASSET_PACK_TEAM_MANAGEMENT_ID, TECMO_ASSET_PACK_TYPE_DATA, 3U,
        TECMO_ASSET_PACK_TEAM_MANAGEMENT_ENTRY_CPU,
        team_management_provenance->starters_flow_offset,
        TECMO_ASSET_PACK_FLAG_DERIVED | TECMO_ASSET_PACK_FLAG_LOCAL);
    if (tecmo_asset_pack_builder_add_memory(
            builder, &entry_info, team_management_payload,
            sizeof(team_management_payload), message, message_size) != 0) {
        tecmo_asset_pack_set_message(
            message, message_size,
    ...
    if (enforce_finale_revision_fingerprints != 0) {
        ...
        entry_info = tecmo_asset_pack_make_entry_info(
            ...
            TECMO_ASSET_PACK_TYPE_DATA, 6U, 0xA125U,
            pretip_provenance->source_offsets[4],
            TECMO_ASSET_PACK_FLAG_DERIVED | TECMO_ASSET_PACK_FLAG_LOCAL);
        if (tecmo_asset_pack_builder_add_memory(
                builder, &entry_info, pretip_payload,
        ...
        entry_info = tecmo_asset_pack_make_entry_info(
            ...
            TECMO_ASSET_PACK_TYPE_DATA, 5U, 0x970BU,
            backcourt_provenance->source_offsets[0U],
            TECMO_ASSET_PACK_FLAG_DERIVED | TECMO_ASSET_PACK_FLAG_LOCAL);
        if (tecmo_asset_pack_builder_add_memory(
                builder, &entry_info, backcourt_payload,
                sizeof(backcourt_payload), message, message_size) != 0) {
            tecmo_asset_pack_set_message(
                message, message_size,
                "Could not write strict TGBC-1 gameplay backcourt entry.");
            return -1;
        }
        ...
        entry_info = tecmo_asset_pack_make_entry_info(
            ...
            TECMO_ASSET_PACK_TYPE_DATA, 5U, 0x879BU,
            gameplay_movement_provenance->source_offsets[2],
            TECMO_ASSET_PACK_FLAG_DERIVED | TECMO_ASSET_PACK_FLAG_LOCAL);
        if (tecmo_asset_pack_builder_add_memory(
                builder, &entry_info, gameplay_movement_payload,
                sizeof(gameplay_movement_payload), message,
                message_size) != 0) {
            tecmo_asset_pack_set_message(
                message, message_size,
                "Could not write strict TGMO-1 gameplay movement entry.");
            return -1;
        }
        entry_info = tecmo_asset_pack_make_entry_info(
        ...
        if (tecmo_asset_pack_builder_add_memory(
            ...
            tecmo_asset_pack_set_message(
                message, message_size,
                "Could not write strict TGAI-3 CPU steering entry.");
            return -1;
        }
        entry_info = tecmo_asset_pack_make_entry_info(
            TECMO_ASSET_PACK_GAMEPLAY_HUD_ID,
            TECMO_ASSET_PACK_TYPE_DATA, 1U, 0xBDF0U,
            gameplay_hud_provenance->source_offsets[0],
            TECMO_ASSET_PACK_FLAG_DERIVED | TECMO_ASSET_PACK_FLAG_LOCAL);
        if (tecmo_asset_pack_builder_add_memory(
                builder, &entry_info, gameplay_hud_payload,
                sizeof(gameplay_hud_payload), message,
                message_size) != 0) {
            tecmo_asset_pack_set_message(
                message, message_size,
                "Could not write strict THUD-1 gameplay HUD entry.");
            return -1;
        }
        entry_info = tecmo_asset_pack_make_entry_info(
        ...
        entry_info = tecmo_asset_pack_make_entry_info(
            TECMO_ASSET_PACK_GAMEPLAY_DUNK_ID,
            TECMO_ASSET_PACK_TYPE_DATA, 5U, 0x856BU,
            dunk_provenance->source_offsets[0],
            TECMO_ASSET_PACK_FLAG_DERIVED | TECMO_ASSET_PACK_FLAG_LOCAL);
        if (tecmo_asset_pack_builder_add_memory(
                builder, &entry_info, dunk_payload,
                sizeof(dunk_payload), message, message_size) != 0) {
            tecmo_asset_pack_set_message(
                message, message_size,
        ...
        entry_info = tecmo_asset_pack_make_entry_info(
            ...
            shot_resolution_provenance->source_offsets[0],
            TECMO_ASSET_PACK_FLAG_DERIVED | TECMO_ASSET_PACK_FLAG_LOCAL);
        if (tecmo_asset_pack_builder_add_memory(
                builder, &entry_info, shot_resolution_payload,
                sizeof(shot_resolution_payload), message,
                message_size) != 0) {
            tecmo_asset_pack_set_message(
                message, message_size,
                "Could not write strict TGSR-4 gameplay entry.");
            return -1;
        }
        entry_info = tecmo_asset_pack_make_entry_info(
        ...
        entry_info = tecmo_asset_pack_make_entry_info(
            ...
            TECMO_ASSET_PACK_FLAG_DERIVED | TECMO_ASSET_PACK_FLAG_LOCAL);
        if (tecmo_asset_pack_builder_add_memory(
                builder, &entry_info, penalty_payload,
                sizeof(penalty_payload), message, message_size) != 0) {
            tecmo_asset_pack_set_message(
                message, message_size,
    ...
    size_t message_size)
{
    ...
    memset(&backcourt_provenance, 0, sizeof(backcourt_provenance));
    ...
    memset(&gameplay_hud_provenance, 0,
           sizeof(gameplay_hud_provenance));
...
int tecmo_asset_pack_self_test(char *message, size_t message_size)
{
    ...
    if (tecmo_asset_pack_gameplay_backcourt_self_test(
            message, message_size) != 0) {
        goto cleanup;
    }
    ...
    if (tecmo_asset_pack_gameplay_hud_self_test(
            message, message_size) != 0) {
        goto cleanup;
    }
    ...
    if (tecmo_asset_pack_gameplay_free_throw_lineup_self_test(
            message, message_size) != 0) {
        goto cleanup;
    }
