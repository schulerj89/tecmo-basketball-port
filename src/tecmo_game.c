...
/* Diagnostic bases used by the conditional $F106-$F1B0 selected-actor clamp.
 * Do not present these as a universal legal movement envelope. */
#define DEBUG_CPU_LEFT_BOUNDARY_BASE 0x00DF
#define DEBUG_CPU_RIGHT_BOUNDARY_BASE 0x0220
    ...
    if (tecmo_load_chr_data(project_root, &runtime->title_chr_bytes, &runtime->title_chr_byte_count) == 0) {
        ...
        if (runtime->team_data_asset.available &&
            !tecmo_team_data_asset_chr_available(&runtime->team_data_asset,
                                                 runtime->title_chr_bytes,
                                                 runtime->title_chr_byte_count)) {
            runtime->team_data_asset.available = false;
...
void tecmo_runtime_set_mode(TecmoRuntime *runtime, TecmoPlayMode mode)
{
    ...
    if (mode == TECMO_MODE_TEAM_DATA) {
        tecmo_team_data_state_init(&runtime->team_data_state);
        if (runtime->team_data_asset.available) {
            runtime->team_data_state.direction_cooldown =
                runtime->team_data_asset.selector_initial_cooldown;
        }
    }
...
static void remember_start_game_menu_return(TecmoRuntime *runtime)
{
    ...
    runtime->start_menu_return_music_value = state->music_value;
    runtime->start_menu_return_speed_value = state->speed_value;
    runtime->start_menu_return_period_index = state->period_index;
}
...
static bool return_to_start_game_menu(TecmoRuntime *runtime)
{
    ...
    music_value = runtime->start_menu_return_music_value;
    speed_value = runtime->start_menu_return_speed_value;
    period_index = runtime->start_menu_return_period_index;
    ...
    if (music_value >= TECMO_START_GAME_MENU_MUSIC_VALUE_COUNT) {
        music_value = 1U;
    }
    if (speed_value >= TECMO_START_GAME_MENU_SPEED_VALUE_COUNT) {
        speed_value = 1U;
    }
    if (period_index >= TECMO_START_GAME_MENU_PERIOD_VALUE_COUNT) {
        period_index = 1U;
    }
    ...
    runtime->start_game_menu_state.music_value = music_value;
    runtime->start_game_menu_state.speed_value = speed_value;
    runtime->start_game_menu_state.period_index = period_index;
...
static void update_team_data_menu(TecmoRuntime *runtime,
                                  const TecmoControlFrame *controls)
{
                                         ...
                                         TecmoFramebuffer *fb)
{
    ...
    if (holder < TECMO_GAMEPLAY_SCENE_ACTOR_COUNT) {
        const TecmoGameplayCpuSteeringRouteMotionState *route =
            &live->play_state.route_motion[holder];
        holder_left_edge = DEBUG_CPU_LEFT_BOUNDARY_BASE -
            scene->actors[holder].position.y / 2;
        holder_right_edge = DEBUG_CPU_RIGHT_BOUNDARY_BASE +
            scene->actors[holder].position.y / 2;
        holder_inside_edges = scene->actors[holder].position.x >=
                holder_left_edge &&
