    ...
    TecmoGameplayCpuOpcode15State7Result *result_out)
{
    ...
    uint8_t actor;
    if (!latch_valid(latch) || input == NULL || output == NULL ||
        result_out == NULL || (const void *)input == (const void *)output ||
    ...
    if (input->actor_state_057c[actor] != 7U) return false;
                                                     ...
                                                     size_t message_size)
{
    ...
    input.primary_0308 = 4U;
    input.actor_state_057c[8U] = 7U;
    input.actor_timer_046e[8U] = 1U;
    if (!tecmo_gameplay_cpu_opcode15_state7_consume(
            &latch, &input, &output, &result) || result.retired_state7 ||
        output.actor_state_057c[8U] != 7U) goto fail;

    latch_before = latch;
    output_before = output;
    memset(&result, 0xA5, sizeof(result));
    result_before = result;
    output.actor_state_057c[8U] = 4U;
    if (tecmo_gameplay_cpu_opcode15_state7_consume(
            &latch, &output, &input, &result) ||
    ...
    (void)snprintf(message, message_size,
                   ...
                   "selector=3 stale=persistent reset=full-only");
    return true;
fail:
    (void)snprintf(message, message_size,
