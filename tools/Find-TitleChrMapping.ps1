...
$Report = [pscustomobject]@{
    ...
    title_setup = [pscustomobject]@{
        ...
        status = "needs BA16 pattern/VRAM load model before raw CHR bank rendering is pixel-accurate"
    }
    ...
    next_steps = @(
        'Model the 04:$BA16 pattern setup path into explicit native pattern-table/VRAM state.',
        "Identify title palette initialization and palette animation state.",
        "Promote the original-title quick launch from probe font to CHR-backed render after BA16 and palette state are mapped."
    )
}
...
[pscustomobject]@{
    ...
    next_gate = "Model 04:`$BA16 pattern setup"
} | Format-List
