...
if ((Get-FileHash -LiteralPath $RomPath -Algorithm SHA256).Hash -ne
    $ExpectedRomSha256) {
    throw "TGBD-1 tests require the exact Tecmo NBA Basketball Rev1 ROM."
}

$BuildDir = Join-Path $ProjectRoot "build"
$Executable = Join-Path $BuildDir "tecmo_port.exe"
$Scratch = [IO.Path]::GetFullPath(
    (Join-Path $BuildDir "gameplay_ball_dribble_test"))
$BuildPrefix = [IO.Path]::GetFullPath($BuildDir).TrimEnd('\', '/') +
    [IO.Path]::DirectorySeparatorChar
if (!$Scratch.StartsWith($BuildPrefix,
        [StringComparison]::OrdinalIgnoreCase)) {
    throw "Gameplay-ball-dribble scratch path escaped build\."
}
$PackPath = Join-Path $Scratch "gameplay-ball-dribble.assetpack"
...
function Invoke-BallDribble([string]$AssetPack, [bool]$Success,
                            [string]$Failure = "") {
    $Output = @(& $Executable --gameplay-ball-dribble-test $AssetPack 2>&1)
    $Text = ($Output -join [Environment]::NewLine)
    if ($Success) {
        if ($LASTEXITCODE -ne 0 -or
            $Text -notmatch 'TGBD-1 held-ball bounce self-test passed') {
            throw "Valid TGBD-1 test failed.`n$(Get-Tail $Output)"
        }
    } elseif ($LASTEXITCODE -eq 0 -or
              $Text -notmatch 'Gameplay ball-dribble test failed:' -or
              ($Failure -and $Text -notmatch [regex]::Escape($Failure))) {
        throw "Invalid TGBD-1 pack was accepted.`n$(Get-Tail $Output)"
    }
}
...
function Write-Mutation([byte[]]$Original, [string]$Name,
    ...
    Invoke-BallDribble $Path $false $Failure
}
...
try {
    ...
    if ($Build) {
        ...
        if ($LASTEXITCODE -ne 0 -or @($BuildOutput | Where-Object {
            ...
            throw "Warning-free TGBD build failed.`n$(Get-Tail $BuildOutput)"
        }
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "TGBD-1 asset-pack build failed.`n$(Get-Tail $PackOutput)"
    }
    $SourceOutput = @(& $Executable --gameplay-ball-dribble-test `
        $PackPath $RomPath 2>&1)
    if ($LASTEXITCODE -ne 0 -or
        ($SourceOutput -join [Environment]::NewLine) -notmatch
            'TGBD-1 held-ball bounce self-test passed') {
        throw "TGBD-1 source/parser vectors failed.`n$(Get-Tail $SourceOutput)"
    }

    $PackBytes = [IO.File]::ReadAllBytes($PackPath)
    $BallDribble = Get-Entry $PackBytes "gameplay/ball-dribble"
    $GameplayCore = Get-Entry $PackBytes "gameplay/core"
    $Movement = Get-Entry $PackBytes "gameplay/movement"
    $SourceMap = Get-Entry $PackBytes "system/source-map"
    $Payload = Get-EntryBytes $PackBytes $BallDribble
    if ($BallDribble.byte_count -ne 608 -or
        (Get-Fnv1a32 $Payload) -ne "E2CE6BFF") {
        throw "TGBD-1 canonical payload changed."
    }
    ...
    if ($Mapped.Count -ne 1 -or
        $Mapped[0].schema -ne "tecmo.gameplay-ball-dribble/TGBD-1" -or
        @($Mapped[0].source_spans).Count -ne 2 -or
        $Mapped[0].source_spans[0].fingerprint_fnv1a32 -ne "DB540670" -or
        $Mapped[0].source_spans[1].fingerprint_fnv1a32 -ne "E9784D28" -or
        @($Mapped[0].dependencies).Count -ne 2 -or
        ![bool]$Mapped[0].native_contract.transactional) {
        throw "TGBD-1 source-map provenance is incomplete."
    }
    Invoke-BallDribble $PackPath $true
    Write-Mutation $PackBytes "header" ([int]$BallDribble.pack_offset) `
        "TGBD-1"
    Write-Mutation $PackBytes "routine" `
        ([int]$BallDribble.pack_offset + 256) "TGBD-1"
    Write-Mutation $PackBytes "tables" `
        ([int]$BallDribble.pack_offset + 416) "TGBD-1"
    Write-Mutation $PackBytes "gameplay-core-dependency" `
        ([int]$GameplayCore.pack_offset) "same-pack"
    Write-Mutation $PackBytes "movement-dependency" `
        ([int]$Movement.pack_offset) "same-pack"

    $Missing = [byte[]]$PackBytes.Clone()
    $Missing[[int]$BallDribble.directory_offset] = [byte][char]'x'
    $MissingPath = Join-Path $Scratch "missing.assetpack"
    [IO.File]::WriteAllBytes($MissingPath, $Missing)
    Invoke-BallDribble $MissingPath $false "missing or wrong-sized"
    $Oversized = [byte[]]$PackBytes.Clone()
    [BitConverter]::GetBytes([uint64]609).CopyTo(
        $Oversized, [int]$BallDribble.directory_offset + 92)
    $OversizedPath = Join-Path $Scratch "oversized.assetpack"
    [IO.File]::WriteAllBytes($OversizedPath, $Oversized)
    Invoke-BallDribble $OversizedPath $false "missing or wrong-sized"
    $global:LASTEXITCODE = 0
    Write-Host "TGBD-1 held-ball bounce tests passed."
} finally {
